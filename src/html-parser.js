...
/* Adapted from https://github.com/developit/htm - Jason Miller */
/*
  - Removed MINI mode
  - Added support for string {{expressions}}
  - Added interpolation() caller function
*/
...
export function evaluate({ h, interpolation = (v) => v }, built, fields, args = []) {
  let tmp;
  ...
  for (let i = 1; i < built.length; i++) {
    ...
    const value = built[i]
      ? ((built[0] |= type ? 1 : 2), interpolation(fields[built[i]], built[i++]))
      : built[++i];
    ...
    if (type === TAG_SET) {
      ...
      // `this` for the `h` call.
      tmp = h.apply(value, evaluate({ h, interpolation }, value, fields, ['', null]));
      args.push(tmp);
